...
<div class="list-table vue">
    ...
    <div class="table">
        ...
        <div v-if="list && list.models && list.models.length" v-for="(item, index) in list.models" class="table-tr table-item" :class="{'link-inside':list.config.link,deletable:list.config.deletable}" @click="liclick(item, $event)" :data-create="item._creating">
            <div v-for="(column,i) in list.columns" class="table-td" v-if="column.visible && (!item.getAction || (item.getAction(item) != item.getAction(list.models[index - 1])))" :class="{'link-inside':typeof column.link == 'string', 'editable':column.editable === true, 'important':item.isImportant()}" v-on:click="click(item,column, $event)">
