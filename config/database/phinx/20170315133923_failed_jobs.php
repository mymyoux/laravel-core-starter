...
class FailedJobs extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    public function changing()
    {
       $this->table('failed_jobs', ['id' => false, 'primary_key' => ['id']])
            ...
            ->addColumn('connection', 'text', [ 'null' => true])
            ->addColumn('queue', 'text', [ 'null' => true])
            ->addColumn('payload', 'text', [ 'limit' => MysqlAdapter::TEXT_LONG,'null' => true])
            ->addColumn('exception', 'text', [ 'limit' => MysqlAdapter::TEXT_LONG,'null' => true])

            ->addColumn('failed_at', 'timestamp', ['default' => 'CURRENT_TIMESTAMP'])
            ->addColumn('id_beanstalkd', 'integer', ['limit' => 11, 'null' => true, 'signed' => false])
