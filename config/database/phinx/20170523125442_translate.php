...
class Translate extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        $this->rollback();
        $this->table('translate')->removeIndex('controller')
        ->update();
        if(!$this->table('translate')->hasColumn('path'))
        $this->table('translate')
        ->addColumn('path','string',['null'=>true])
        ->update();
         
         $this->execute('UPDATE translate SET path = CONCAT(controller,".",action,".",`key`)');
         $this->table('translate')
         ->addIndex('path',['unique'=>false])
        ->addIndex(['path','type','locale'],['unique'=>true])
        ->update();
         //dd('ok');
    }
