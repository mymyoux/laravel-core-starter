...
class ComentRelation extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    public function changing()
    {
        ...
        if($this->hasTable('comment_relation_user'))
            $this->dropTable('comment_relation_user');
        if($this->hasTable('comment_relation'))
            $this->dropTable('comment_relation');

        if($this->table('comment')->hasColumn('external_id'))
        {
                $this->table('comment')
                ->removeColumn('external_id')
                ->removeColumn('external_type');
        }
        if(!$this->hasTable('comment_relation'))
        {
           $this->table('comment_relation', ['id' => false, 'primary_key' => ['id_comment_relation']])
            ->addColumn('id_comment_relation', 'integer', ['limit' => 11, 'null' => false, 'signed' => false,'identity'=>True])
            ->addColumn('name', 'string', ['null' => False,'limit'=>1000])
        ...
        if(!$this->table('comment')->hasColumn('id_comment_relation'))
        {
            $this->table('comment')
            ->addColumn('id_comment_relation','integer',['limit' => 11, 'null' => false, 'signed' => false])
            ->addForeignKey('id_comment_relation','comment_relation','id_comment_relation', $this->actions)
            ->update();
        }
        if(!$this->hasTable('comment_relation_user'))
        {
            $this->table('comment_relation_user', ['id' => false, 'primary_key' => ['id_comment_relation_user']])
                ->addColumn('id_comment_relation_user', 'integer', ['limit' => 11, 'null' => false, 'signed' => false,'identity'=>True])
                ->addColumn('id_comment_relation', 'integer',['limit'=>11,'null'=>False,'signed'=>False])
                ->addColumn('external_id', 'integer',['limit'=>11,'null'=>False,'signed'=>False])
                ->addColumn('external_type', 'string', ['limit' => 255, 'null' => false])
                ->addColumn('created_time', 'timestamp', ['null' => true])
                ->addColumn('updated_time', 'timestamp', ['null' => true, 'default' => 'CURRENT_TIMESTAMP', 'update' => 'CURRENT_TIMESTAMP'])
                ->addForeignKey('id_comment_relation','comment_relation','id_comment_relation', $this->actions)
                ->create();
