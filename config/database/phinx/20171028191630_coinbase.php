...
class Coinbase extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    public function changing()
    {
        if(!$this->isRollback())
        {
            $this->execute('DELETE FROM connector WHERE name="coinbase"');
            $this->table('connector')->insert(["name"=>"coinbase","is_primary"=>1])->saveData();
        }

        $this->table('user_connector_coinbase', ['id' => false, 'primary_key' => ['id_user','id']])
