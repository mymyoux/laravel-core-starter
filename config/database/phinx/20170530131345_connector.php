...
class Connector extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        $this->rollback();
        $this->execute("CREATE TABLE `connector` (
  `id_connector` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL DEFAULT '',
  `scopes` varchar(2000) DEFAULT NULL,
  `is_primary` int(1) NOT NULL DEFAULT '0',
  `created_time` timestamp NULL DEFAULT NULL,
  `updated_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_connector`),
  KEY `name` (`name`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;");

  $this->execute("CREATE TABLE `user_connector` (
  `id_user` int(11) unsigned NOT NULL,
  `id_connector` int(11) unsigned NOT NULL,
  ...
  `created_time` timestamp NULL DEFAULT NULL,
  `updated_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_user`,`id_connector`),
  KEY `id_connector` (`id_connector`),
  CONSTRAINT `user_connector_ibfk_1` FOREIGN KEY (`id_user`) REFERENCES `user` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `user_connector_ibfk_2` FOREIGN KEY (`id_connector`) REFERENCES `connector` (`id_connector`) ON DELETE CASCADE ON UPDATE CASCADE
    ...
    {
        if($this->hasTable('user_connector'))
        {
            $this->dropTable('user_connector');
        }
        if($this->hasTable('connector'))
        {
