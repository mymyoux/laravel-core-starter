...
class CommentRelation extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        $this->table('comment_relation')
        ->addColumn('count','integer',['signed'=>False,'null'=>True,'limit'=>11])
        ->update();
        if(!$this->isRollback())
        {
            $this->execute('UPDATE comment_relation SET count=(SELECT COUNT(DISTINCT id_comment_relation_user) FROM comment_relation_user WHERE comment_relation_user.id_comment_relation = comment_relation.id_comment_relation GROUP BY comment_relation_user.id_comment_relation)');
        }
    }
