...
class ConnectorSlack extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        $this->rollback();
        $this->table('connector')->insert(["name"=>"slack","is_primary"=>1])
        ->saveData();


           $this->table('user_connector_slack', ['id' => false, 'primary_key' => ['id_user','id']])
            ->addColumn('id_user', 'integer', ['limit' => 11, 'null' => false, 'signed' => false])
            ->addColumn('id', 'string', ['limit' => 200, 'null' => false])
            ->addColumn('name', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('nickname', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('email', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('avatar', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('team_name', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('gender', 'integer', ['limit' => 1, 'null' => true,'signed'=>False])
            ->addColumn('id_slack_user', 'string', ['limit' => 2000, 'null' => true])
            ->addColumn('id_slack_team', 'string', ['limit' => 256, 'null' => true])
            ->addColumn('created_time', 'timestamp', ['null' => true])
            ->addColumn('updated_time', 'timestamp', ['null' => true])
            ->create();
        
        
        //execute('INSERT INTO connector VALUES(NULL,"slack",NULL,1,NULL,NULL)');
     //   dd('ok');
    }
    public function rollback()
    {
         $this->execute('DELETE FROM connector WHERE name="slack"');
         if($this->hasTable('user_connector_slack'))
         {
             $this->dropTable('user_connector_slack');
         }
    }
