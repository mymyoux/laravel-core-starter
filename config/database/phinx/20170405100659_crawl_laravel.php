...
class CrawlLaravel extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        if($this->hasTable("crawl"))
        {
            if($this->table('crawl')->hasColumn('version') && !$this->isRollback())
                return;
            $this->table("crawl")
            ->addColumn('version','string',['limit'=>255,'default'=>'zend', 'null'=>true])
            ->update();
        }
    }
