...
class FixIndexes extends AbstractMigration
{
    /**
     * Change Method.
     *
   ...
   public  $actions = ['delete'=> 'CASCADE', 'update'=> 'CASCADE'];
    ...
    {
        $this->rollback();
         $result = $this->fetchAll('SELECT table_name FROM information_schema.tables where table_schema="'.config('database.connections.mysql.database').'"');
        foreach($result as $tablerow)
        {
          $table = $this->table($tablerow["table_name"]);
          if($table->hasColumn('created_time'))
          {
            if(!$table->hasIndex('created_time'))
            {
              $this->output->writeln('create index created_time for '.$tablerow["table_name"]);
              $table->addIndex('created_time', ['unique'=>False])->update();
            }
          }
          if($table->hasColumn('updated_time'))
          {
            if(!$table->hasIndex('updated_time'))
            {
              $this->output->writeln('create index updated_time for '.$tablerow["table_name"]);
              $table->addIndex('updated_time', ['unique'=>False])->update();
            }
          }
        }
    }
