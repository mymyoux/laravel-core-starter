...
class Clean extends CoreCommand
{
    ...
    public function handle()
    {
        ...
        Call::where("created_at","<",DB::raw("NOW() - INTERVAL $months MONTH"))->delete();
        if($all)
        {
            Logger::info("deleting beanstalkd_log");
            Log::where("created_at","<",DB::raw("NOW() - INTERVAL $months MONTH"))->delete();
        }
        Logger::info("deleting error");
        Error::where("created_at","<",DB::raw("NOW() - INTERVAL $months MONTH"))->delete();
        Logger::info("deleting stats_watch");
        Watch::where("created_at","<",DB::raw("NOW() - INTERVAL $months MONTH"))->delete();
        Logger::info("deleting ats_api_error");
        AtsApiCall::where("created_at","<",DB::raw("NOW() - INTERVAL $months MONTH"))->delete();
        Logger::info("done");
    }
